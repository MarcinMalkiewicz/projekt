...
import com.marcin.formObjects.BlockForm;
...
private ObjectMapper mapper;
	...
	public RedirectView newUserPost(HttpSession session,Register form) {
		...
		if(session.getAttribute("logged").toString()!="LOGGED") {
			...
			User x = users.findByEmail(form.getEmail());
	...
	@PostMapping("/block")
	public RedirectView block( String email,BlockForm form, HttpSession session) {
		if(session.getAttribute("logged").equals("LOGGED")){
		        User blockedUser = users.findByEmail(form.getEmail());
		        if (blockedUser!=null) {
		      
		         users.blockUser("BLOCKED",blockedUser.getEmail()); 
		         return new RedirectView("/");
		        }
